...
if __name__ == "__main__":
    ...
    # Perform the grid search on the tuned parameters (GridSearchCV object)
    model = GridSearchCV(SVC(C=1), tuned_parameters, cv=10)
    model.fit(X_train, y_train)
    
    # Outputs results of the Grid Search/Cross-Validation procedure:
    print("Optimized parameters found on training set:")
    print(model.best_estimator_, "\n")
    
    print("Grid scores calculated no training set:")
