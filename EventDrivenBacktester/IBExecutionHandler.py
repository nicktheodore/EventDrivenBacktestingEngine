...
from EventDrivenBacktester.ExecutionHandler import ExecutionHandler
...
class IBExecutionHandler(ExecutionHandler):
    """
    Handles order execution via the Interactive Brokers API, for use agains accounts when trading live directly.
    """
    ...
    def create_fill(self, msg):
        """
        Handles the creation of the FillEvent that will be placed onto the events queue
        subsequent to an order being filled.
        """
    ...
    def execute_order(self, event):
        ...
        The results are then queried in order to generate a corresponding Fill object, which is
        ...
        if event.type == 'ORDER':
            ...
            # NOTE: THE FOLLOWING LINE IS CRUCIAL. IT ENSURES THE ORDER GOES THROUGH!
            time.sleep(1)
