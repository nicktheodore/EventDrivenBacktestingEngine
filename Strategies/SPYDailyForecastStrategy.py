...
class SPYDailyForecastStrategy(Strategy):
    """
    S&P500 Forecasting strategy. It uses a Quadratic Discriminant Analyzer to predict the returns
    for a subsequent time period and the generated long/exit signals based on the prediction.
    """
    ...
    def create_symbol_forecast_model(self):
        ...
        # Change the following line to use a different model (RF, SVM, Logits)
        model = QDA()
...
if __name__ == "__main__":
    ...
    backtest = Backtest(csv_dir, symbol_list, initial_capital, heartbeat, start_date,
                        HistoricCSVDataHandler, SimulatedExecutionHandler, Portfolio, SPYDailyForecastStrategy)
